...
def solve(q0, box_position=np.array([1.5, 0.0, 0.6 - PANDA_BASE_HEIGHT]), desired_theta=0, verbose=True):
    ...
    dz = box_position[2] - AE[2] - bottle_height * abs(np.cos(desired_theta))  # - bottle_height / 2
    ...
    def delayed(vx, vy):
        # external force after release
        ft = flying_time(vy)
        omega = dtheta / ft

        l2 = m1 * L / (m1 + m2)
        alpha = np.arctan2(vy + omega * l2 * np.sin(theta_E),
                           vx - omega * l2 * np.cos(theta_E)) - np.pi  # assume the gripper is stopped
        # backward
        vx -= forcetime / (m1 + m2) * np.sin(alpha)
        vy -= forcetime / (m1 + m2) * np.sin(alpha)
        omega -= -forcetime / (m2 * L) * np.sin(theta_E - alpha)
        return [vx, vy, omega, alpha]

    def qdot4full(fvels):
        vels = np.array([[fvels[0], 0, fvels[1], 0, fvels[2], 0]]).transpose()
        q_dot = np.linalg.pinv(J) @ vels
        return q_dot

    def delayedqdot(vx, vy):
        fvels = delayed(vx, vy)
        return qdot4full(fvels)
    ...
    res = minimize(obj, x_init, constraints=(
        ...
        {'type': 'ineq', 'fun': lambda x: q_dot_ul * 0.9 - delayedqdot(x[0], x[1])[:, 0]},
        {'type': 'ineq', 'fun': lambda x: q_dot_ul * 0.9 + delayedqdot(x[0], x[1])[:, 0]},
    ))
    vx_r, vy_r = res.x
    vx, vy, omega, alpha = delayed(vx_r, vy_r)
    q_dot = qdot4full([vx, vy, omega])[:, 0]
    ft = flying_time(vy_r)
    landx = AE[0] + vx * ft

    deviation = dr - vx * ft
    if verbose:
        print('raw vx:{:.3f}, vy:{:.3f}, omega:{:.3f}, alpha:{:.3f}'.format(vx_r, vy_r, dtheta / ft, alpha))
        print('vx:{:.3f}, vy:{:.3f}, omega:{:.3f}, flying time:{:.3f}'.format(vx, vy, omega, ft))
...
def validate(q0, q0_dot):
    # validate the solution from practice
    ...
    dz = box_position[2] - AE[2] - bottle_height * abs(np.cos(0))  # - bottle_height / 2
    dr = box_position[0] - AE[0]
    dtheta = 0 - theta_E
    if 0 <= dtheta < np.pi:
        dtheta -= 2 * np.pi
    ...
    vels = J @ np.array(q0_dot)
    vx, _, vy, _, omega, _ = vels
    l2 = m1 * L / (m1 + m2)
    alpha = np.arctan2(vy + omega * l2 * np.sin(theta_E),
                       vx - omega * l2 * np.cos(theta_E)) - np.pi  # assume the gripper is stopped
    # backward
    ...
    # find avaliable vx, vy
    ft = -vy / gravity + np.sqrt((vy / gravity) ** 2 + 2 * dz / gravity)

    delta_theta = ft * omega

    print('raw vx vy omega {:.3f} {:.3f} {:.3f}'.format(vels[0], vels[2], vels[4]))
    print('delayed vx, vy, omega {:.3f} {:.3f} {:.3f}'.format(vx, vy, omega))
    print('delta_theta real/expected', delta_theta, dtheta)
...
if __name__ == '__main__':
    ...
    #validate(np.array([0.0, 0.4, 0.0, -0.7, 0.0, 2.02, 0.0 + 45 / 180 * np.pi]),
    #         np.array([0, -1.10508170520, 0, 2.15504300e+00, 0.0, 2.5504305e+00, 0]))

    q0 = np.array([0., 1.5, 0., -0.6, 0., 3.4, 0.78539816])
    #q0 = np.array([0., 1.3, 0., -0.6, 0., 3.1, 0.78539816])
    # q0 = np.array([0., 1.2, 0., -1., 0., 3.6, 0.78539816])
    ...
    if succ:
        ...
        pybullet.disconnect()
        vis(res[0], res[1], landx=res[5] + 0.4, slow=1)  # , video_path='bottle_flip.mp4')
    else:
